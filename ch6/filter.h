...
public:
    ...
    // 根据类型来生成模板
    cv::Mat gen_gaussian_kernel(const int &ksize, const double &sigma);
    cv::Mat gen_mean_kernel(const int &ksize);
    cv::Mat gen_laplacian_kernel(const int &ksize);
    cv::Mat gen_sobel_kernel(int axis);
