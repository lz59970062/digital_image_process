        ...
        for (auto i = 0; i < img2.rows; i++)
        {
            ...
            for (auto j = 0; j < img2.cols; j++)
            {
                ...
                case (1):
                    ...
                    if (p2.at<float>(0) > 0 && p2.at<float>(1) > 0 && p2.at<float>(0) < img.cols && p2.at<float>(1) < img.rows)
                    {
                        ptr[j] = img.at<Vec3b>(p2.at<float>(1), p2.at<float>(0));
                    }
                    else
                    {
                        ptr[j] = Vec3b(0, 0, 0);
    ...
    { 
        for (auto i = 0; i < img.rows; i++)
        {
            ...
            for (auto j = 0; j < img.cols; j++)
            {
                Mat p = (Mat_<float>(3, 1) << j, i, 1);
                Mat p2 = T * p;
                if (p2.at<float>(0) > 0 && p2.at<float>(1) > 0 && p2.at<float>(0) < img2.cols && p2.at<float>(1) < img2.rows)
                {
                    img2.at<Vec3b>(p2.at<float>(1), p2.at<float>(0)) = ptr[j];
                }
            }
        }
    }
