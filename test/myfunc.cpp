...
Vec3b inster2(const Mat& img, double x, double y) // 最近邻插值
{
    ...
    int yi = round(y);
    xi = std::max(0, xi);
    xi = std::min(img.cols - 1, xi);
    yi = std::max(0, yi);
    yi = std::min(img.rows - 1, yi);
    Vec3b r = img.at<Vec3b>(yi, xi);
    return r;
}

Vec3b inster3(const Mat& img, double x, double y) // 双线性插值
{
    double x1 = floor(x);
    double x2 = ceil(x);
    double y1 = floor(y);
...
Vec3b inster3(const Mat& img, float x, float y) {
    int x1 = floor(x);
    int y1 = floor(y);

    x1 = min(max(x1, 1), img.cols - 3);
    y1 = min(max(y1, 1), img.rows - 3);
    ...
    Vec3b result;
    for (int i = 0; i < 3; i++) {
        float col0 = cubic_interp(img.at<Vec3b>(y1 - 1, x1 - 1)[i], img.at<Vec3b>(y1 - 1, x1)[i], img.at<Vec3b>(y1 - 1, x1 + 1)[i], img.at<Vec3b>(y1 - 1, x1 + 2)[i], dx);
        float col1 = cubic_interp(img.at<Vec3b>(y1, x1 - 1)[i], img.at<Vec3b>(y1, x1)[i], img.at<Vec3b>(y1, x1 + 1)[i], img.at<Vec3b>(y1, x1 + 2)[i], dx);
        float col2 = cubic_interp(img.at<Vec3b>(y1 + 1, x1 - 1)[i], img.at<Vec3b>(y1 + 1, x1)[i], img.at<Vec3b>(y1 + 1, x1 + 1)[i], img.at<Vec3b>(y1 + 1, x1 + 2)[i], dx);
        float col3 = cubic_interp(img.at<Vec3b>(y1 + 2, x1 - 1)[i], img.at<Vec3b>(y1 + 2, x1)[i], img.at<Vec3b>(y1 + 2, x1 + 1)[i], img.at<Vec3b>(y1 + 2, x1 + 2)[i], dx);
        result[i] = clamp(cubic_interp(col0, col1, col2, col3, dy), 0.0f, 255.0f);
    }
    return result;
}
...
R:rotation mat
*/
Mat img_transf(Mat &img, Mat &T, int method)
{
        ...
        for (auto i = 0; i < img2.rows; i++)
        {
            ...
            for (auto j = 0; j < img2.cols; j++)
            {
                ...
                case (0):
                    ...
                    {
                        ptr[j] = inster1(img, p2.at<float>(1), p2.at<float>(0)); // 双线性插值
                    }
                    else
                    {
                        ptr[j] = Vec3b(0, 0, 0);
                ...
                case (1):
                    if (p2.at<float>(0) > 0 && p2.at<float>(1) > 0 && p2.at<float>(0) < img.cols && p2.at<float>(1) < img.rows)
                    {
                        ptr[j] = inster2(img, p2.at<float>(1), p2.at<float>(0));
                    }
                    else
                    {
                        ptr[j] = Vec3b(0, 0, 0);
                ...
                case 2:
                    if (p2.at<float>(0) > 0 && p2.at<float>(1) > 0 && p2.at<float>(0) < img.cols && p2.at<float>(1) < img.rows)
                    {
                        ptr[j] = inster3(img, p2.at<float>(1), p2.at<float>(0));
                    }
                    else
                    {
                        ptr[j] = Vec3b(0, 0, 0);
